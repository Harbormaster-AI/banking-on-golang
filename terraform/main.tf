...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "b16a0fdc-efae-471f-985e-2ead4a281114"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.033813644962302325"
...
resource "aws_db_subnet_group" "default" {
  name       = "bankingOnGolang-db-subnet"
...
#  name        = "bankingOnGolang-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingOnGolang created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for bankingOnGolang and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "bankingOnGolang-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "bankingOnGolang"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_bankingOnGolang"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "bankingOnGolang instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-on-golang:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingOnGolang theharbormaster/banking-on-golang:latest"
    ]
  }
}
